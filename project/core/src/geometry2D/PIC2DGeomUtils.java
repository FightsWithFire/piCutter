...
public class PIC2DGeomUtils {
	...
	static PICPoint2D intersect(PICPoint2D s1, PICPoint2D e1, PICPoint2D s2, PICPoint2D e2) {
		...
		if (Math.abs(rcs) < PICTolerance.ZERO_LENGTH) {
			if (qpr < PICTolerance.ZERO_LENGTH) {
				...
				if (t0 < 1 + PICTolerance.ZERO_LENGTH && t0 > - PICTolerance.ZERO_LENGTH) {
					t0 = Math.max(0, t0);
					t1 = Math.min(t1, 1);
					// TODO: the proper result in the case is actually a line segment. I'm just returning one of its end points at random.
			...
			double u =  s1.subtract(s2).cross(r) / rcs;
			double t = s2.subtract(s1).cross(s) / rcs;
			// TODO: I should be using something related to the lengths of the vectors rather than a static tolerance.
			if (-PICTolerance.ZERO_LENGTH < u && u < 1 + PICTolerance.ZERO_LENGTH &&
					-PICTolerance.ZERO_LENGTH < t && t < 1 + PICTolerance.ZERO_LENGTH) {
